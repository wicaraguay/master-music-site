...
export const Navigation: React.FC<NavigationProps> = ({ currentSection, onNavigate, lang, setLang }) => {
  ...
  return (
    <nav
      className={`fixed top-0 left-0 w-full z-50 transition-all duration-700 border-b border-transparent ${
        isScrolled
    ...
    >
        ...
        <div className="hidden lg:flex space-x-8 items-center">
          {navStructure.map((item) => {
            if (item.isSubmenu && item.children) {
              return (
                <div key={item.id} className="relative group">
                  <button 
                    className={`flex items-center gap-1 text-[10px] xl:text-xs tracking-[0.2em] uppercase transition-all duration-300 hover:text-maestro-gold ${
                      isTrayectoriaActive
                        ? 'text-maestro-gold font-bold border-b border-maestro-gold pb-1'
                        : 'text-maestro-light/70'
                    }`}
                  >
                    {item.label}
                    ...
                    <div className="bg-maestro-dark border border-white/10 p-2 min-w-[180px] shadow-2xl rounded-sm">
                      {item.children.map(child => (
                        <button
                          ...
                          className={`block w-full text-left px-4 py-3 text-[10px] uppercase tracking-[0.15em] hover:bg-white/5 hover:text-maestro-gold transition-colors ${
                            currentSection === child.id ? 'text-maestro-gold' : 'text-maestro-light/70'
                          }`}
                        >
                          {child.label}
                        </button>
            ...
            return (
              <button
                ...
                className={`text-[10px] xl:text-xs tracking-[0.2em] uppercase transition-all duration-300 hover:text-maestro-gold ${
          ...
          <div className="relative">
            <button 
                ...
                className="text-[10px] xl:text-xs tracking-[0.2em] uppercase transition-all duration-300 text-maestro-light/70 hover:text-maestro-gold flex items-center gap-2 border-l border-white/10 pl-6"
            >
          ...
          <button 
            ...
            className={`text-[10px] xl:text-xs tracking-[0.2em] uppercase transition-all duration-300 hover:text-maestro-gold flex items-center gap-2 border border-white/10 px-3 py-1 rounded-sm ${
      ...
      {/* Mobile Menu Overlay */}
      <div
        className={`fixed inset-0 bg-black z-40 flex flex-col pt-32 px-10 space-y-6 transition-transform duration-700 cubic-bezier(0.7, 0, 0.3, 1) lg:hidden overflow-y-auto ${
      ...
      >
        ...
        {navStructure.map((item) => {
             ...
             return (
               <div key={item.id} className="flex flex-col items-start w-full border-b border-white/5 pb-4">
                 <button 
                   ...
                   className={`text-3xl font-serif transition-colors duration-300 flex items-center justify-between w-full ${
                     isTrayectoriaActive ? 'text-maestro-gold' : 'text-maestro-light'
                   }`}
                 >
                   {item.label}
                   <ChevronDown 
                 ...
                 `}>
                    {item.children.map(child => (
                      <button
                        key={child.id}
                        onClick={() => handleNavigate(child.id)}
                        className={`text-lg font-serif text-left transition-colors duration-300 ${
                          currentSection === child.id ? 'text-maestro-gold italic' : 'text-maestro-light/60'
                        }`}
                      >
                        — {child.label}
                      </button>
           ...
           return (
            <button
              key={item.id}
              onClick={() => handleNavigate(item.id as Section)}
              className={`text-3xl font-serif text-left transition-colors duration-300 border-b border-white/5 pb-4 w-full ${
                currentSection === item.id ? 'text-maestro-gold italic' : 'text-maestro-light'
              }`}
            >
              {item.label}
            </button>
        ...
        <button
          onClick={() => handleNavigate(Section.ADMIN)}
          className={`text-xl font-serif text-left transition-colors duration-300 pt-4 w-full flex items-center gap-3 ${
            currentSection === Section.ADMIN ? 'text-maestro-gold italic' : 'text-maestro-light/50'
          }`}
        >
